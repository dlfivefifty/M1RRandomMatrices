...
Unfortunately, there is no simple formula for the relationship between the moments of $A$ and $B$ and those of the mixed moments, but they can be derived systematically like we did for the first four moments. Note the cyclic property of trace applies for free probability too:

\begin{lemma}[free cyclic property] Mixed moments have a cyclic property:
\[
...
\]
\end{lemma}
\textbf{Proof}  This is a simple proof by induction that be discussed in a poster.

\ensuremath{\QED}

We can used this to help deduce all moments up to degree 4 using the above calculations:
\meeq{
...
Unfortunately there is no simple combinatorial formula for these expressions. But there is a miraculous equivalent way of constructing them in terms of the \emph{Stieltjes transform}:

\begin{definition}[Stieltjes transform] The Stieltjes transform of a free random variable is the following moment generating function:
\[
G_A(z) := {1 \over z} + {\ensuremath{\bm{\mathrm{E}}} A \over z^2} + {\ensuremath{\bm{\mathrm{E}}} A^2  \over z^3} + \ensuremath{\cdots}.
\]
It is also called the Cauchy transform. \end{definition}
...
\]
Addition of free random variables can be reduced to a simple formula involving the \emph{R-transform}:

\begin{definition}[R-transform] The \emph{R-transform} is defined in terms of the inverse of the Stieltjes transform as follows:
\[
...
\begin{theorem}[addition of free random variables] The free convolution can be expressed in terms of the R-transform as follows:
\[
R_{A+B}(z) = R_A(z) + R_B(z).
\]
\end{theorem}
\textbf{Proof} Proving this is a very serious combinatorial problem connected to noncrossing partitions, which could be an excellent poster project. \ensuremath{\QED} 
