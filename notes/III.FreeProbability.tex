...
\textbf{Example (adding two semicircles)} We conclude with an example where we can do everything explicitly. Suppose $A$ and $B$ are the large $n$ limits of two indepedent Wigner ensembles. We know that $A+B$ is also the large end limit of a Wigner ensemble (but with double the variance) so will also have a semicircle eigenvalue distribution. How do we see this fact come out of free convolution? In this case the moments are identical:
\[
...
We therefore have
\[
\tilde{G}_A(z) \ensuremath{\equiv} \tilde{G}_B(z) = z + \ensuremath{\sum}_{k=2}^\ensuremath{\infty} \ensuremath{\bm{\mathrm{E}}} A^{k-1} z^k = C_0 z + C_1 z^3 + C_2 z^4 + \ensuremath{\cdots}.
\]
