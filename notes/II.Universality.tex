...
\]
\end{proposition}
\textbf{Proof} We know that a symmetric matrix is \emph{diagonalisable}:
\[
...
\[
C_m := {1 \over m+1} \begin{pmatrix} 2m \\ m \end{pmatrix} =  {(2m)! \over (m+1)! m!}.
\]
\end{definition}
...
\item[4. ] The number $1$ cannot be paired with $5$ since we cannot pair $\{2,3,4\}$ without having a left over.

\end{itemize}
Thus we see the pattern: $1$ cannot be paired with any odd number since then there will always be one left over. Otherwise, if $1$ is paired with an even number $2k$ we have to independently pair the $2(k-1)$ numbers $\{2,\ensuremath{\ldots},2k-1\}$ and the $2(m+1-k)$ numbers $\{2k+1,\ensuremath{\ldots},2(m+1)\}$. Thus there are $C_{k-1} C_{m+1-k}$ possibilities.

Putting all this together we have the total number of pairings:
\begin{align*}
\underbrace{C_m}_{\hbox{$\{1,2\}$ is a pair}} &+ 
\underbrace{C_{m-1}}_{\hbox{$\{1,4\}$ is a pair}} + 
\underbrace{C_2 C_{m-2}}_{\hbox{$\{1,6\}$ is a pair}} +
\underbrace{C_3 C_{m-3}}_{\hbox{$\{1,8\}$ is a pair}} \\
& + \ensuremath{\cdots} + 
...
&= {2^{k+1} \over \ensuremath{\pi}} \ensuremath{\int}_0^\ensuremath{\pi} \cos^k \ensuremath{\theta} \sin^2 \ensuremath{\theta} \D\ensuremath{\theta} = {2^{k+1} \over \ensuremath{\pi}} \ensuremath{\int}_0^\ensuremath{\pi} \cos^k \ensuremath{\theta} (1-\cos^2 \ensuremath{\theta}) \D\ensuremath{\theta} \\
&= {2^{k+1} \over \ensuremath{\pi}} \br[ \ensuremath{\int}_0^\ensuremath{\pi} \cos^k \ensuremath{\theta} \D\ensuremath{\theta} - \ensuremath{\int}_0^\ensuremath{\pi} \cos^{k+2} \ensuremath{\theta} \D\ensuremath{\theta}].
}
We claim that
\[
\ensuremath{\int}_0^\ensuremath{\pi} \cos^{2m} \ensuremath{\theta} \D\ensuremath{\theta} = {\ensuremath{\pi} \over 2^{2m}} \begin{pmatrix}
2m \\ m
\end{pmatrix}
\]
...
\[
A^k \ensuremath{\bm{\e}}_j = \Vectt[\ensuremath{\sum}_{i_2,\ensuremath{\ldots},i_k} a_{1 i_2} a_{i_2 i_3} \ensuremath{\cdots} a_{i_k j}, \ensuremath{\vdots}, \ensuremath{\sum}_{i_2,\ensuremath{\ldots},i_k} a_{n i_2} a_{i_2 i_3} \ensuremath{\cdots} a_{i_k j}].
\]
But now we just sum over $j$ to get the trace:
\[
\ensuremath{\sum}_{j=1}^n \ensuremath{\bm{\e}}_j^\ensuremath{\top} A^k \ensuremath{\bm{\e}}_j = \ensuremath{\sum}_{j=1}^n \ensuremath{\sum}_{i_2,\ensuremath{\ldots},i_k} a_{j i_2} a_{i_2 i_3} \ensuremath{\cdots} a_{i_k j} = \ensuremath{\sum}_{i_1,\ensuremath{\ldots},i_k = 1}^n a_{i_1 i_2} a_{i_2 i_3} \ensuremath{\cdots} a_{i_k i_1}.
\]
\ensuremath{\QED}
...
\]
Now this clearly simplifies, but to prepare for the general case we again break the sum into two different cases: either the two terms equal ($a_{i_1i_2} = a_{i_1i_2}$ or $a_{i_1i_2} = a_{i_2i_1}$) or they differ. We associate these two cases with the partitions $\{1,2\}$ (terms match) and $\{1\}, \{2\}$ (terms differ).  But in this case all terms fall into the matching category:
\[
\begin{array}{c | c | c}
...
We thus have 
\[
{\ensuremath{\bm{\mathrm{E}}} A_n^2 \over n} = {1 \over n^2} \ensuremath{\sum}_{i_1,i_2=1}^n \ensuremath{\bbE}[\underbrace{a_{i_1i_2} a_{i_2i_1}}_{\hbox{always match}}]  = 1 = C_1.
\]
For $k = 3$ we consider different cases of writing
\[
{\ensuremath{\bm{\mathrm{E}}} A_n^3 \over n^{3/2}} = {1 \over n^{5/2}} \ensuremath{\sum}_{i_1,i_2,i_3=1}^n \ensuremath{\bbE}[a_{i_1i_2}a_{i_2i_3}a_{i_3i_1}].
\]
...
\hline
  ...
  \{1,3\}, \{2\}  & \ensuremath{\bbE}[a_{i_1i_2} a_{i_2i_2} a_{i_2i_1}] = \ensuremath{\bbE}[a_{i_1i_2}^2 a_{i_2i_2}] = \ensuremath{\mu}_2 \ensuremath{\mu}_1 = 0 &  n (n-1)= n^3 + O(n^2)  \\
...
\meeq{
{\ensuremath{\bm{\mathrm{E}}} A_n^4 \over n^2} = {1 \over n^3} \ensuremath{\sum}_{i_1,i_2,i_3,i_4=1}^n \ensuremath{\bbE}[a_{i_1i_2}a_{i_2i_3}a_{i_3i_4}a_{i_4i_1}]
}
