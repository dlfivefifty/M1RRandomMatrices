...
We now establish the moments of the semicircle distribution, using a combinatorial construction with a similar flavour (but not the same!) as the relationship between the normal distribution and double factorials. In the case of the semicircle the key object are the Catalan numbers $C_m$ which play a very similar row as $(2m-1)!!$:
...
We thus have 
\[
{\ensuremath{\bm{\mathrm{E}}} A_n \over n} = {1 \over n^2} \ensuremath{\sum}_{i_1,i_2=1}^n \ensuremath{\bbE}[\underbrace{a_{i_1i_2} a_{i_2i_1}}_{\hbox{always match}}]  = 1 = C_1.
\]
