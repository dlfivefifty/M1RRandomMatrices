...
\]
i.e., a sum with $n^k$ terms. Since we know $X_i$ commute, one angle of proof is to simplify this using the multinomial expansion to combine the terms where the different $i_\ensuremath{\ell}$ match, but instead we are going to break up the sum according to whether the components are the same or different, which can be identified with partitions of $\{1,\ensuremath{\ldots},n\}$.
...
For $k = 3$ we consider
\[
\ensuremath{\bbE}S_n^3 = n^{-3/2} \ensuremath{\sum}_{i_1,i_2,i_3=1}^n \ensuremath{\bbE}[X_{i_1}X_{i_2}X_{i_3}]
\]
...
At this point the argument is clear for general $k$. There are $n^k + O(n^{k-1})$ terms where each element differ, corresponding to the partition $\{1\},\ensuremath{\ldots},\{k\}$, but because of independence these all contribute zero because independence guarantees:
\[
