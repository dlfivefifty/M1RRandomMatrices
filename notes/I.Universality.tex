...
\hline
  \{1\}, \{2\}, \{3\}  & \ensuremath{\bbE}[a_{kj} a_{j\ensuremath{\ell}} a_{\ensuremath{\ell}k}] = 0   &  n (n-1)(n-2) = n^3 + O(n^2) \\
  \{1,2\}, \{3\}  & \ensuremath{\bbE}[a_{kj} a_{jk} a_{kk}] = \ensuremath{\mu}_2 \ensuremath{\mu}_1 = 0  &  n (n-1)= n^3 + O(n^2) \\
  \{1,3\}, \{3\}  & \ensuremath{\bbE}[a_{kj} a_{jj} a_{jk}] = \ensuremath{\mu}_2 \ensuremath{\mu}_1 = 0 &  n (n-1)= n^3 + O(n^2)  \\
  \{1\},  \{2,3\}  & \ensuremath{\bbE}[a_{kk} a_{k\ensuremath{\ell}} a_{\ensuremath{\ell}k}] = \ensuremath{\mu}_2 \ensuremath{\mu}_1 = 0 &  n (n-1)= n^3 + O(n^2)  \\
 \{1,2,3\} & \ensuremath{\bbE}[a_{kk} a_{kk} a_{kk}] = \ensuremath{\mu}_3&  n.
\end{array}
\]
...
We can again divide the sum up by which terms are equal, which are one-to-one with the number of partitions of 4. Explicitly, for $i,j, k$ and $\ensuremath{\ell}$  never equal we have the following cases:
\[
\begin{array}{c | c | c}
\hbox{Partition} & \hbox{Term} & \hbox{Count} \\
\hline
\{1\}, \{2\}, \{3\}, \{4\} & \ensuremath{\bbE}[a_{ik} a_{kj} a_{j\ensuremath{\ell}} a_{\ensuremath{\ell}i}] = 0 & n (n-1)(n-2)(n-3) = n^4 + O(n^2) \\
\{1,2\}, \{3\}, \{4\} & \hbox{Not possible!} & 0 \\
\{1,3\}, \{2\}, \{4\} & \ensuremath{\bbE}[a_{ik} a_{kk} a_{ki} a_{ii}] = 0 & n (n-1) = n^2 + O(n) \\
\{1,4\}, \{2\}, \{3\} & \hbox{Not possible!} & 0 \\
\{1,2\}, \{3,4\} & \ensuremath{\bbE}[a_{ik} a_{ki} a_{ij} a_{ji}] = \ensuremath{\mu}_2^2 = 1 & n (n-1)(n-2) = n^3 + (n^2) \\
\{1,3\}, \{2,4\} & \hbox{Not possible!} & 0 \\
\{1,4\}, \{2,3\} & \ensuremath{\bbE}[a_{ik} a_{kj} a_{jk} a_{ki}] = \ensuremath{\mu}_2^2 = 1 & n (n-1)(n-2) = n^3 + (n^2) \\
\{1,2,3\}, \{4\} & \hbox{Not possible!} & 0 \\
\{1,2,4\}, \{3\} & \hbox{Not possible!} & 0 \\
\{1\}, \{2,3,4\} & \hbox{Not possible!} & 0 \\
\{1,2,3,4\} & \ensuremath{\bbE}[a_{kk} a_{kk} a_{kk} a_{kk}] = \ensuremath{\mu}_4 & n
\end{array}
\]
Thus the moment reduces to:
\meeq{
\ensuremath{\bm{\mathrm{E}}}\br[(A_n/\sqrt{n})^4] = {1 \over n^3} \underbrace{\ensuremath{\sum}_{i=1}^n \ensuremath{\bbE}[a_{ii}^4]}_{= O(n)} +
{1 \over n^3} \underbrace{\ensuremath{\sum}_{i=1}^n \ensuremath{\sum}_{j\ensuremath{\neq}i} \ensuremath{\bbE}[a_{ij}^4]}_{=O(n^2)} \\
&\qquad +
{1 \over n^3} \underbrace{\ensuremath{\sum}_{i=1}^n \ensuremath{\sum}_{j\ensuremath{\neq}i} \ensuremath{\sum}_{k \ensuremath{\neq} i,j}  \ensuremath{\bbE}[a_{ij}^2 a_{kj}^2]}_{= n^3 + O(n^2) }  +
{1 \over n^3} \underbrace{\ensuremath{\sum}_{i=1}^n \ensuremath{\sum}_{j\ensuremath{\neq}i} \ensuremath{\sum}_{k \ensuremath{\neq} i,j}  \ensuremath{\bbE}[a_{ij}^2 a_{ik}^2]}_{= n^3 + O(n^2) }\\
&\underset{n \ensuremath{\rightarrow} \ensuremath{\infty}}{\ensuremath{\longrightarrow}} 2.
}
Which matches the Catalan number $C_2$. 

Now for the key insight: when we break apart the sum which has $n^4$ terms, most of the terms are where all 4 are different, but these disappear. After this, the next largest terms are those corresponding to non-crossing pairs of $k$, in this case
\[
(\{1,2\}, \{3,4\}) \qquad (\{1,4\}, \{2,3\})
\]
Each of these contributes exactly $n^3 + O(n^2)$, and hence as $n \ensuremath{\rightarrow} \ensuremath{\infty}$ the moment tends to the number of non-crossing pairs of $k$.

This pattern holds true for all $k$. When $k$ is odd, we have $n^k$ total terms which we can divide up according to the partitions of $k$ numbers. But the only ones that actually contribute are those with no single element in a partition. We omit the details but what remains grows slower than $n^{-k/2-1}$ and hence the moment tends to zero. When $k$ is even, the only terms that grow precisely like $n^{-k/2-1}$ are those corresponding to non-crossing pair partitions of $k$. Which we know equals the Catalan numbers!
...
\item[2. ] Hermitian random matrices also satisfy universality properties. How do histograms of their eigenvalue statistics (both global and local) compare to the symmetric case?


\item[3. ] A \emph{Wishart ensemble} is a random symmetric matrix $X X^\ensuremath{\top}$ where $X \ensuremath{\in} \ensuremath{\bbR}^{m \ensuremath{\times} n}$ is a rectangular matrix with random entries. It also exhibits universality but depending on the ratio $\ensuremath{\alpha} = m/n$. Can you explore how the global and local statistics behave? Do they match the Wigner ensemble case?


\item[4. ] The spacing of zeros of the Riemann Zeta function on the critical line match the universality. to higher accuracy?


\item[5. ] What happens when the 4-moment condition breaks down? I.e. what are eigenvalue statistics for matrices with heavytailed entries?


\item[6. ] One can consider matrices associated with random networks, a la as discussed in Introduction to Applied Mathematics. How connected does a random network need to be to exhibit universality?
