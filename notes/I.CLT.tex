...
\]
\end{theorem}
\textbf{Proof} We are going to mimick the proof of the semicircle law as much as possible. We first note we can naively expand
\[
(X_1 + \ensuremath{\cdots} + X_n)^k = \ensuremath{\sum}_{i_1,\ensuremath{\ldots},i_k=1}^n X_{i_1} \ensuremath{\cdots} X_{i_k},
\]
i.e., a sum with $n^k$ terms. Since we know $X_i$ commute, one angle of proof is to simplify this using the multinomial expansion to combine the terms where the differen $i_\ensuremath{\ell}$ match, but instead we are going to divide the sum into different partitions to capture this in a more combinatorial way.
...
For $k = 2$ we consider two different cases: either $i_1 = i_2$ or $i_1 \ensuremath{\neq} i_2$.  We can associate these with partitions of $2$ numbers as $\{1,2\}$ (when they match) or $\{1\}, \{2 \}$ (when they differ). Here is a table of which partitions correspond to which terms, and how many of each term we have, where now we assume $i_1 \ensuremath{\neq} i_2$:
\[
\begin{array}{c | c | c}
\hbox{Partition} & \hbox{Term} & \hbox{Count} \\
\hline
  \{1\}, \{2\}  & \ensuremath{\bbE}[X_{i_1} X_{i_2}] =\ensuremath{\bbE}X_{i_1} \ensuremath{\bbE} X_{i_2}  = 0  & n(n-1) = n^2 + O(n)\\
  \{1,2\}  & \ensuremath{\bbE} X_{i_1}^2 = 1 &  n
\end{array}
\]
Note since $n^2 = n + n(n-1)$ we have accounted for all terms. But the only terms that are non-zero are those associated with $\{1,2\}$. Thus we find:
\[
\ensuremath{\bbE} S_n^2 = n^{-1} \ensuremath{\sum}_{i_1=1}^n  \ensuremath{\bbE} X_{i_1}^2 = 1.
\]
For $n = 3$ we can similarly divide the cases according to the different partitions of 3 elements based on which terms match, where  now we assume $i_1 \ensuremath{\neq} i_2 \ensuremath{\neq} i_3$:
\[
\begin{array}{c | c | c}
\hbox{Partition} & \hbox{Term} & \hbox{Count} \\
\hline
  \{1\}, \{2\} , \{3\}  & \ensuremath{\bbE}[X_{i_1} X_{i_2} X_{i_3}] =\ensuremath{\bbE}X_{i_1} \ensuremath{\bbE} X_{i_2} \ensuremath{\bbE} X_{i_3}  = 0  &  n(n-1)(n-2) = n^3 + O(n^2) \\
  \{1,2\}, \{3\}  & \ensuremath{\bbE}[X_{i_1}^2 X_{i_3}] =\ensuremath{\bbE}X_{i_1}^2 \ensuremath{\bbE} X_{i_3}  = 0  & n(n-1) \\
  \{1,3\}, \{2\}  & \ensuremath{\bbE}[X_{i_1}^2 X_{i_2}] =\ensuremath{\bbE}X_{i_1}^2 \ensuremath{\bbE} X_{i_3}  = 0  & n(n-1) \\
  \{1\}, \{2,3\}  & \ensuremath{\bbE}[X_{i_1} X_{i_2}^2] =\ensuremath{\bbE}X_{i_1} \ensuremath{\bbE} X_{i_2}^2  = 0  & n(n-1) \\
  \{1,2,3\}  & \ensuremath{\bbE} X_{i_1}^3 = 1   &  n 
\end{array}
\]
...
\hbox{Partition} & \hbox{Term} & \hbox{Count} \\
\hline
\{1\}, \{2\}, \{3\}, \{4\} & \ensuremath{\bbE}[X_{i_1} X_{i_2} X_{i_3} X_{i_4}] = 0 & n (n-1)(n-2)(n-3) = n^4 + O(n^2) \\
\{1,2\}, \{3\}, \{4\} & \ensuremath{\bbE}[X_{i_1}^2 X_{i_3} X_{i_4}] = 0 & n(n-1)(n-2)= n^3 + O(n^2) \\
\{1,3\}, \{2\}, \{4\} & \ensuremath{\bbE}[X_{i_1}^2 X_{i_2} X_{i_4}] = 0 & n(n-1)(n-2) = n^3 + O(n^2)\\
\{1,4\}, \{2\}, \{3\} & \ensuremath{\bbE}[X_{i_1}^2 X_{i_2} X_{i_3}] = 0 & n(n-1)(n-2) = n^3 + O(n^2)\\
\{1\}, \{2,3\}, \{4\} & \ensuremath{\bbE}[X_{i_1} X_{i_2}^2 X_{i_4}] = 0 & n(n-1)(n-2) = n^3 + O(n^2)\\
\{1\}, \{2,4\}, \{3\} & \ensuremath{\bbE}[X_{i_1} X_{i_2}^2 X_{i_3}] = 0 & n(n-1)(n-2) = n^3 + O(n^2)\\
\{1\}, \{2\}, \{3,4\} & \ensuremath{\bbE}[X_{i_1} X_{i_2} X_{i_3}^2] = 0 & n(n-1)(n-2) = n^3 + O(n^2)\\
\{1,2\}, \{3,4\} &  \ensuremath{\bbE}[X_{i_1}^2 X_{i_3}^2] = 1 & n(n-1)=n^2 + O(n) \\
\{1,3\}, \{2,4\} &  \ensuremath{\bbE}[X_{i_1}^2 X_{i_2}^2] = 1 & n(n-1)=n^2 + O(n) \\
\{1,4\}, \{2,3\} & \ensuremath{\bbE}[X_{i_1}^2 X_{i_2}^2] = 1 & n(n-1)=n^2 + O(n) \\
\{1,2,3\}, \{4\} & \ensuremath{\bbE}[X_{i_1}^3 X_{i_4}] = 0 & n(n-1) = n^2 + O(n) \\
\{1,2,4\}, \{3\} & \ensuremath{\bbE}[X_{i_1}^3 X_{i_3}] = 0 & n(n-1) = n^2 + O(n) \\
\{1\}, \{2,3,4\} & \ensuremath{\bbE}[X_{i_1} X_{i_2}^3] = 0 & n(n-1) = n^2 + O(n) \\
\{1,2,3,4\} & \ensuremath{\bbE}[X_{i_1}^4] \ensuremath{\neq} 0 & n
\end{array}
\]
...
\meeq{
\ensuremath{\bbE} S_n^4 = n^{-2} \ensuremath{\sum}_{\hbox{pair partitions}} \underbrace{n(n-1)}_{\hbox{terms per partition}}\\
&\qquad  + n^{-2} \ensuremath{\sum}_{\hbox{parititions with 4 elements}} \underbrace{n}_{\hbox{terms per partition}}    \ensuremath{\bbE}[X_{i_1}^4] \\
&=
n^{-2} \underbrace{3}_{\hbox{number of pair partitions}} (n^2 + O(n)) + n^{-1} \ensuremath{\bbE}[X_{i_1}^4]\\
& \underset{n \ensuremath{\rightarrow} \ensuremath{\infty}}{\ensuremath{\longrightarrow}} 3!!
}
At this point the argument is clear for general $k$. There are $n^k + O(n^{k-1})$ partitions where each element differs, but because of independence these all contribute zero because indepedence guarantees:
\[
\ensuremath{\bbE}[X_{i_1} \ensuremath{\cdots} X_{i_k}] = \ensuremath{\bbE}X_{i_1} \ensuremath{\cdots} \ensuremath{\bbE}X_{i_k} = 0
\]
Each partition with a single pair has $n^{k-1} + O(n^{k-2})$ terms, but also vanishes by the same argument. For even $k$ this continues until we reach exactly $k/2$ pairs, in which case the moments are, eg.,
\[
\ensuremath{\bbE}[X_{i_1}^2 \ensuremath{\cdots} X_{i_{k/2}}^2] = \ensuremath{\bbE}X_{i_1}^2 \ensuremath{\cdots} \ensuremath{\bbE}X_{i_{k/2}}^2 = 1,
\]
...
\[
\ensuremath{\bbE} S_n^k \underset{n \ensuremath{\rightarrow} \ensuremath{\infty}}{\ensuremath{\longrightarrow}} 0.
\]
\ensuremath{\QED}
